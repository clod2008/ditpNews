...
export const NavBar = ( {navData, brand, expand} ) => {
  ...
  return (
    ...
    <Navbar key={expand} expand={expand} className={`bg-body-tertiary mb-3 ${styles.mainNavBar}`}>
      <Container fluid>
        <NavLink to={'/'}>
            <Navbar.Brand className={styles.customBrand}><Image src={brand} className='img-fluid' alt='DITP logo' height='35px' width='auto'/></Navbar.Brand>
        </NavLink>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title className={styles.customBrand} id={`offcanvasNavbarLabel-expand-${expand}`}>
              <Image src={brand} className='img-fluid' alt='DITP logo' height='35px' width='auto' />
            </Offcanvas.Title>
          </Offcanvas.Header>
          <Offcanvas.Body>
