...
4. **`api/log-premio.js`** — mismo patrón que `api/log-scan.js`: función serverless que agrega una fila a la pestaña **"Premios"** del mismo Google Sheet que ya usa `GOOGLE_SHEET_ID` (no hace falta una planilla nueva, solo crear esa pestaña si no existe). Rango `Premios!A:I`, columnas en este orden — **timestamp** (momento exacto del sorteo), **premio** (nombre del tipo entregado), **participante** (nombre libre, opcional — input en la pantalla pública, agregado 2026-08-07), **encargado** (obligatorio, se define una sola vez en el admin y persiste en `sorteo_config` — agregado 2026-08-07), **stockRestante** (cuánto queda de ESE premio después de esta entrega), **plataforma** (`android`/`ios`/`desktop`, mismo criterio que `SmartLink.jsx`), **dispositivoId** (uuid persistente por dispositivo — se genera una sola vez en `localStorage`, `getDeviceId()` en `Sorteo.jsx`, y se reusa siempre; agrupa todas las entregas del mismo aparato incluso entre los 4 días del evento, no identifica a la persona), **id** (uuid del sorteo puntual, dedup) y **origin** (metadata técnica). Headers descriptivos a pegar a mano en la fila 1 de la pestaña: `Fecha y hora | Premio entregado | Participante | Encargado del sorteo | Stock restante | Plataforma | ID de dispositivo | ID de sorteo | Origen` — el `values:append` de la API detecta solo la última fila con contenido y escribe después, nunca pisa la fila de headers. Reusa las mismas tres env vars server-side que `log-scan.js` — nada nuevo que configurar en Vercel para esta parte.
...
- Crear la pestaña "Premios" en el Google Sheet si todavía no existe. Si ya existía con la fila de headers vieja (`A:G`), agregar a mano `Participante` y `Encargado del sorteo` en `H1`/`I1` — la API no los crea sola.
